...
export function createNoise(seed: number = Math.random()) {
  ...
  // 2D Simplex Noise
  function getSimplex2d(x: number, y: number) {
    ...
    const returnValue = 70 * (n0 + n1 + n2);

    // Convert from -1...1 to 0...1
    return (1 + returnValue) / 2;
  }

  // 2D Perlin Noise
  function getPerlin2d(x: number, y: number) {
  ...
  return { getPerlin2d, getSimplex2d, updateSeed };
}
